...
The final pipeline was built and trained on the NYC Airbnb dataset. Hyperparameter
tuning was performed using Hydra's multirun feature, sweeping over `max_depth`,
`n_estimators`, and `max_tfidf_features`. The best configuration found was:

- `max_depth`: 15
- `n_estimators`: 300
- `max_tfidf_features`: 75

This model was tagged `prod` in Weights & Biases and evaluated on the held-out test set:
